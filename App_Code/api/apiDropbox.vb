...
Public Module apiDropbox
    ...
    Public Class DropboxFolder
        Dim blah As New DropNet.Models.MetaData
    ...
    Public Class DropboxFile
        ...
        Public Property DropboxTimeStamp As Date
            Get
                Dim str As String = _
                   ...
                   "   AND [UserID] = " & HttpContext.Current.Session("user.UserID") & _
                   " ORDER BY [Modified] DESC"
                Dim dt As New DataTable
                SelectRows(dt, str)
                If dt.Rows.Count = 0 Then
                    Throw New FileNotFoundException("Undefined version for FileID " & theFileID & " and UserID " & HttpContext.Current.Session("user.UserID"))
                End If
        ...
        Public ReadOnly Property IamTheOwner As Boolean
            Get
                Return (HttpContext.Current.Session("user.UserID") = dr.Field(Of Integer)("OwnerID"))
            End Get
        End Property
        ...
        'For existing file
        Public Sub New(ByRef FileID As Integer)
            NewByID(FileID)
        End Sub
        'For existing file, but retrieve by Path, rather than FileID
        Public Sub New(ByRef DirectoryPath As String, ByRef Filename As String)
        ...
        Public Sub New(ByRef thePathAndFilename As String)
        ...
        Public Sub New(ByRef DirectoryPath As String, ByRef Filename As String, ByRef DropboxTimestamp As Date)
    ...
    Public Sub logDropbox(ByRef ActivityType As DropboxStates, ByRef Filename As String, ByRef Path As String, Optional ByRef Message As String = "")
        logDropbox(HttpContext.Current.Session("user.UserID"), ActivityType, Filename, Path, Message)
    End Sub
    'Log Dropbox File Activity
    ...
    Public Class dropboxUser
        'Internal Variables
        '==========================================================
        Private Dropbox As DropNet.DropNetClient 'xxx Change to "Private" before going live!
        ...
        Private Sub UpdateUserTimeStamp(ByRef DropboxPath As String, ByRef Filename As String)
            'Var
            Dim theServerFile As New DropboxFile(DropboxPath & Filename)
        ...
        Public Function uploadFile(ByRef LocalFile As FileInfo, Optional ByRef DropboxPath As String = "/") As Boolean
            ...
            Dim tStart As Date
            Dim tStop As Date
            logDropbox(apiDropbox.DropboxStates.Uploading, blah.FullName, DropboxPath)
            Try
                tStart = Now
                ...
                If Response = System.Net.HttpStatusCode.OK Then
                    ...
                    logDropbox(apiDropbox.DropboxStates.UploadComplete, blah.FullName, DropboxPath, "Wrote " & content.Length & " Bytes in " & TransferTime.TotalSeconds & " seconds (" & (content.Length / 1024) / TransferTime.TotalSeconds & " kB/s).")
                    UpdateUserTimeStamp(DropboxPath, LocalFile.Name) 'Update modified date
                    Return True
                Else
                    'Failure
                    logDropbox(apiDropbox.DropboxStates.UploadFailed, blah.FullName, DropboxPath, "Result: " & Response.ToString)
                    Err.Raise(-342, , "Upload Failed. " & Response.ToString)
                    Return False
                End If
            Catch ex As Exception
                logDropbox(apiDropbox.DropboxStates.UploadFailed, blah.FullName, DropboxPath, "Exception: " & ex.Message)
                Throw New IOException("Upload of " & blah.FullName & " failed.") 'In the future this might be removed and true handling added... don't know...
                Return False
            End Try
        End Function
        ...
        Public Function downloadFile(ByRef DropboxPathAndFilename As String, ByRef LocalPathAndFilename As String, Optional ByRef Which_Webpage As System.Web.UI.Page = Nothing) As Boolean
            ...
            Try
                ...
                tStop = Now
                Dim TransferTime As New TimeSpan(tStop.Ticks - tStart.Ticks)
                logDropbox(apiDropbox.DropboxStates.DownloadComplete, PaF.GetFilename, PaF.GetPath, "Wrote " & theFileData.Length & " Bytes in " & TransferTime.TotalSeconds & " seconds (" & (theFileData.Length / 1024) / TransferTime.TotalSeconds & " kB/s).")
                logUserActivity(Which_Webpage, suGetUserFirstName() & " updated " & PaF.GetPathSubDirsOnly & PaF.GetFilename)
                Return True
            Catch ex As Exception
                logDropbox(apiDropbox.DropboxStates.DownloadFailed, PaF.GetFilename, PaF.GetPath, "Exception: " & ex.Message)
                Err.Raise(-9855, , "Download Failed!")
                Return False
            End Try
        End Function
        ...
        Public Sub SyncFile(ByRef UserDbxFile As DropNet.Models.MetaData)
            ...
            If UserDbxFile.Is_Dir = False Then
                If UserDbxFile.Is_Deleted = False Then
                    ...
                    Dim ServerFile As New DropboxFile(theDropboxPath.GetPath, theFilename)
                    ...
                    If UserDbxFile.ModifiedDate.CompareTo(ServerFile.DropboxTimeStamp) > 0 Then
                        ...
                        Else
                            ...
                            logDropbox(DropboxStates.ErrorUnauthorizedChanges, theFilename, UserDbxFile.Path, "User changed file without permission. Authorized version has been restored.")
                        End If
                        '=============================================================================================================
                        ...
                        For Each userFile As DropNet.Models.MetaData In theDirMeta.Contents
                            ...
                            Catch ex As FileNotFoundException
                                ...
                                If fplibValidExtension(userFile.Extension, userFile.Path) Then
                                    ...
                                    Dim newFile As New DropboxFile(New txtPathAndFilename(userFile.Path).GetPath, userFile.Name, userFile.ModifiedDate) 'Save metadata
